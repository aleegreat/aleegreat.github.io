...
# 思路
LXC是特权容器：按Linux常规挂载即可
LXC不是特权容器：远程目录挂载到PVE，通过PVE和LXC目录映射

# 方案（仅思路2）
获取Archllinux中用户的uid、gid（一般为1000，1000）
获取PVE与LXC中uid、gid映射起始值
```bash
...
1. mount命令中的uid、gid 等于 lxc 容器中uid、gid （即1000，1000）加上 pve与lxc映射值（即100000，100000）等于 101000，101000
2. 在pve中mount好远程目录
```bash
...
1.  在LXC容器中创建好映射目录
2. 修改LXC容器的配置文件 `nano /etc/pve/lxc/yourlxcid.conf` ，新增一行
```bash
